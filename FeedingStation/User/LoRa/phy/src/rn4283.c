...
int rn2483_init(rn2483_st* dev, serial_port_st* serialPort)
{
...
#ifdef TARGET
    HAL_GPIO_WritePin(RN2483_EN_GPIO_Port, RN2483_EN_Pin, GPIO_PIN_RESET);
    THREAD_SLEEP_FOR(SYSTEM_TICK_FROM_MS(5));
    HAL_GPIO_WritePin(RN2483_EN_GPIO_Port, RN2483_EN_Pin, GPIO_PIN_SET);
#endif

    serial_flush(serialPort);
    ...
    ret = serial_readline(serialPort, dev->serialBuffer, BUFFER_SIZE, MAX_DELAY);
    if (ret < 0)
        return -EFAULT;
...
int rn2483_tx(rn2483_st* dev, const uint8_t* pSrc, uint8_t srcLen)
{
    ...
    ret = serial_readline(dev->serialPort, dev->serialBuffer, BUFFER_SIZE, MAX_DELAY);
    if (ret < 0)
        return -EFAULT;
...
int rn2483_set_wdt(rn2483_st* dev, uint64_t timeout)
{
    ...
    sprintf((char*)dev->serialBuffer, "radio set wdt %llu\r\n", timeout);
